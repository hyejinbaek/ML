'''
knn imputation method(missing value 20%)
'''
...
df_tt = df_data.drop(['id'],axis='columns')
df_total = df_tt.loc[:139] = np.nan
imputer = KNNImputer(n_neighbors=2)
df_im = imputer.fit_transform(df_data)

df_ttt= pd.DataFrame(df_im)
col_data = df_ttt.columns = ['id', 'Clump Thickness', 'Uniformity of Cell Size', 'Uniformity of Cell Shape',
                              'Marginal Adhesion ', 'Single Epithelial Cell Size',
                              'Bare Nuclei', 'Bland Chromatin', 'Normal Nucleoli', 'Mitoses', 'Class']
df_t = df_ttt.drop(['id'],axis='columns')

###model###
train_col = ['Clump Thickness', 'Uniformity of Cell Size', 'Uniformity of Cell Shape',
                              'Marginal Adhesion ', 'Single Epithelial Cell Size',
